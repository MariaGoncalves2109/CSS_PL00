...
import pt.ul.fc.css.example.demo.dtos.NewAuthorDTO;
...
public class AuthorController {
  ...
  public AuthorDTO getAuthor(@PathVariable Long id) {
    ...
    return new AuthorDTO(author.getId(), author.getName(), author.getSurname(), getBookIds(author.getBooks()));
  }
    ...
    return this.authorRepository.findAll().stream()
        .map(
            author ->
                ...
                author.getSurname(), getBookIds(author.getBooks())))
        .toList();
  }
  ...
  @PostMapping("/author")
  public Long postAuthor(@NonNull @RequestBody NewAuthorDTO author) {
  ...
  @PutMapping("/author")
  public Long putAuthor(@NonNull @RequestBody AuthorDTO author) {
    Optional<Author> optionalAuthor = authorRepository.findById(author.id());
    if (optionalAuthor.isPresent()) {
        return updateAuthor(author, optionalAuthor.get());
    }
    return addAuthor(new NewAuthorDTO(author.name(), author.surname()));
  }

  private Long updateAuthor(AuthorDTO author, Author existingAuthor) {
    if(author.name() != null && !author.name().isBlank())
      existingAuthor.setName(author.name());
    if(author.surname() != null && !author.surname().isBlank())
      existingAuthor.setSurname(author.surname());
    Author saved = authorRepository.save(existingAuthor);
    return saved.getId();
  }

  private Long addAuthor(NewAuthorDTO author) {
    Author toSave = new Author(author.name(), author.surname());
